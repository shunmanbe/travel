...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <form action="/itineraries/{{ $detail->id }}/second_destination_store" method="POST">
            @csrf
            <h2>{{$place[1]}}</h2>
            <input type="hidden" name="second[second_destination_address]" value={{$place[0]}}>
            <input type="hidden" name="second[second_destination_name]" value={{$place[1]}}>
